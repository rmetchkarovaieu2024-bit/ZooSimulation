...
class Database:
    ...
    def _create_tables(self):
        self.conn.executescript("""
            -- Persistent animal registry: animals live across runs
            CREATE TABLE IF NOT EXISTS animals (
                ...
                is_alive        INTEGER NOT NULL DEFAULT 1,  -- 1=alive, 0=dead
            ...
            -- Per-run animal snapshot (hunger + health at end of each day)
            CREATE TABLE IF NOT EXISTS animal_state (
                ...
                event           TEXT DEFAULT 'snapshot'  -- 'snapshot','died','born'
            );
    ...
    def register_animals(self, animals: list):
        """
        Called at zoo build time.
        - If an animal already exists in DB (same name + species): load its
          saved hunger/health so state carries over from the last run.
        - If it's new: insert it and return defaults.
        Returns a dict {animal_id: animal} after syncing state.
        """
        id_map = {}
        for a in animals:
            ...
            if row:
                ...
                # New animal — insert into registry
                c = self.conn.cursor()
                c.execute("""
                    INSERT INTO animals
            ...
            self.conn.commit()
            id_map[animal_id] = a
            a._db_id = animal_id   # attach DB id to animal object

        return id_map

    def save_animal_state(self, animals: list, flagged_names: set):
        """Save end-of-day state snapshot for every living animal."""
        if not self.current_run_id:
            return
        for a in animals:
            ...
            # Update persistent record with latest state
            if db_id:
                self.conn.execute("""
                    UPDATE animals SET
    ...
    def record_death(self, animal, cause: str = "starvation"):
        """Mark an animal as dead in the persistent registry."""
    ...
    def record_birth(self, offspring, mother, father=None) -> int:
        """Insert a newborn animal into the persistent registry."""
        ...
        log("DATABASE",
            ...
            f"id={offspring._db_id}  "
            f"mother='{mother.name}'", GREEN)
        return offspring._db_id

    def living_animals(self) -> list:
        """Return all currently living animals from the registry."""
    ...
    def animal_history(self, name: str) -> list:
        """Full state history of a specific animal across all runs."""
        rows = self.conn.execute("""
    ...
    def print_animal_registry(self):
        """Print all living animals with their persistent state."""
